...
class nsgaAlgo(object):
    ...
    def __init__(self, 
             instance_path='./data/json/Input_Data.json',
             pop_size=400, 
             cross_prob=0.85, 
             mut_prob=0.02, 
             num_gen=150):
    
        self.json_instance = load_instance(instance_path)
    ...
    def doExport(self):
        csv_file_name = f"{self.json_instance['instance_name']}_{self.algorithm_name}_" \
...
class nsga3Algo(nsgaAlgo):
    """
    Implementation of NSGA-III by inheriting from the base nsgaAlgo class.
    We only need to override the __init__ to change the selection operator
    and doExport to change the output filename.
    """
    def __init__(self, 
             ref_points_p=99, 
             **kwargs): # Use **kwargs to pass other args to parent
    
    # Call parent __init__ with all the standard arguments
        super().__init__(**kwargs) 
        
        self.algorithm_name = "NSGA3"
        self.nobj = 2 
        self.ref_points_p = ref_points_p 
        self.ref_points = tools.uniform_reference_points(self.nobj, self.ref_points_p)
        
        # OVERRIDE the selection operator
        self.toolbox.register("select", partial(tools.selNSGA3, ref_points=self.ref_points))

    # ... This is inside the class nsga3Algo(nsgaAlgo):

    # We MUST override runGenerations because the parent class uses
    # tools.selTournamentDCD, which is specific to NSGA-II (crowding_dist)
    def runGenerations(self):
        # Running algorithm for given number of generations
        for gen in range(self.num_gen):
            print(f"{20*'#'} Currently Evaluating {gen} Generation {20*'#'}")
            ...
            # Select parents using a standard binary tournament (tournsize=2).
            # This will use the fitness values (rank and reference point distance)
            # that are correctly calculated by selNSGA3.
            self.offspring = tools.selTournament(self.pop, len(self.pop), tournsize=2)
            ...
            # Recalcuate the population with newly added offsprings and parents
            # We are using NSGA3 selection method, We have to select same population size
            self.pop = self.toolbox.select(self.pop + self.offspring, self.pop_size)
    ...
    def doExport(self):
        # Override to add "NSGA3" to the filename
        csv_file_name = f"{self.json_instance['instance_name']}_{self.algorithm_name}_" \
                        f"pop{self.pop_size}_crossProb{self.cross_prob}" \
                        f"_mutProb{self.mut_prob}_numGen{self.num_gen}.csv"
        exportCsv(csv_file_name, self.logbook)
...
if __name__ == "__main__":
    print("Running file directly. We will execute and compare NSGA-II and NSGA-III.")
    
    # --- Run NSGA-II ---
    print(f"\n{40*'-'}\n" \
          f"     RUNNING NSGA-II ALGORITHM" \
          f"\n{40*'-'}")
    ...
    # This is critical. DEAP's creator functions are global.
    # We must delete the old types before registering new ones
    # for the next algorithm run (even though they are the same,
    # it's good practice and prevents errors if they were different).
    try:
    ...
    # --- Run NSGA-III ---
    print(f"\n{40*'-'}\n" \
          f"     RUNNING NSGA-III ALGORITHM" \
          f"\n{40*'-'}")
    
    # Note: for 2 objectives, p=99 divisions creates 100 reference points.
    # You can tune this value.
    nsga3_run = nsga3Algo(ref_points_p=99) 
    nsga3_run.runMain()
    ...
    print("\n--- NSGA-II Best Individual ---")
    print(f"Fitness: {nsga2_run.best_individual.fitness.values}")
    
    print("\n--- NSGA-III Best Individual ---")
    print(f"Fitness: {nsga3_run.best_individual.fitness.values}")
    
    print(f"\nResults for NSGA-II saved to its CSV file.")
    print(f"Results for NSGA-III saved to its CSV file.")
    print("You can now visually compare the Pareto fronts from both CSV files.")
